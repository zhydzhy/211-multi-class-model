...
train_loader = DataLoader(train_data, batch_size=128, shuffle=True,pin_memory=True,collate_fn = collate_fn)
val_loader = DataLoader(val_data, batch_size=128, shuffle=False,pin_memory=True,collate_fn = collate_fn)
...
def evaluate_model(model, data_loader, device):
        ...
        for images, targets in data_loader:
            ...
            for t in targets:
                if t is not None and isinstance(t, list) and len(t) > 0 and isinstance(t[0], dict):
                    labels = get_label_vector(t[0]["image_id"])
    ...
    return precision, recall, f1, subset_accuracy, avg_loss
...
for epoch in range(num_epochs):
    ...
    precision, recall, f1, subset_accuracy, val_loss = evaluate_model(model, val_loader, device)
    print(f"Validation Precision: {precision:.4f}, Recall: {recall:.4f}, F1: {f1:.4f}, Subset Accuracy: {subset_accuracy:.4f}, Val Loss: {val_loss:.4f}")
